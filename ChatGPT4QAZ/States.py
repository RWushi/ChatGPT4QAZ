...
class PostgresStateStorage(BaseStorage):
    ...
    async def get_state(self, *, chat: int = None, user: int = None, default: str = None) -> str:
        ...
        state = await conn.fetchval('SELECT user_state FROM user_settings WHERE user_id = $1', user_id)
        await conn.close()
    ...
    async def set_state(self, *, chat: int = None, user: int = None, state: str = None):
        user_id = user or chat
        conn = await self.create_connection()
        await conn.execute('UPDATE user_settings SET user_state = $2 WHERE user_id = $1', user_id, state)
        await conn.close()
